...
function getIsCanAccess(accessId?: string | number) {
    ...
    const accessIdList = useAppSelector(state => state.user.permissionList);
...
function dict(dict_name: string, key: string) {
    const list:any = [];
    let value = "";
    const tempmap = new Map<string,string>(list[dict_name])
    value = tempmap.get(key) || key
    return value
}
export {
    getH5Env, // 获取h5域名环境
